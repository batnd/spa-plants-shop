...
export class OrderComponent implements OnInit {
  ...
  constructor() {
    // Чтобы валидация была привязана к способу доставки по умолчанию В МОМЕНТ ОТКРЫТИЯ СТРАНИЦЫ (Создания компонента)
  ...
  ngOnInit(): void {
    ...
    // Запрашиваем личные данные с backend и подставляем их в форму заказа (при наличии) - Только если юзер залогинен!
  ...
  public createOrder(): void {
    // Необходима проверка наличия значения (что в поле не undefined!)
    if (this.orderForm.valid && this.orderForm.value.firstName
      && this.orderForm.value.lastName && this.orderForm.value.phone
      ...
      this.orderService.createOrder(paramsObject)
        .subscribe({
          ...
          error: (errorResponse: HttpErrorResponse): void => {
            ...
            else this._snackBar.open('Ошибка заказа');
          }
        })
    } else {
